%extracts scalp maps of each patient at 1 ms intervals from 0 - 400 ms
...
function dtab = extract_erp(sdir, bidsdir, pipeline_erp, doplot)
    ...
    for subid = 13:44
       ...
       % Modify time_intervals to have 1 ms intervals from 0 ms to 400 ms
time_intervals = 0:0.008:0.4; % Millisecond time intervals from 0 ms to 400 ms

num_milliseconds = numel(time_intervals);

for idx = 1:num_milliseconds
    % Extract the topography for each 1 ms interval
    t_start = time_intervals(idx);
    t_end = t_start + 0.008; % 1 ms interval
